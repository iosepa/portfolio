...
function shrinkNav(scrollPos) {
  ...
  if (document.getElementById("popout").classList.contains("popped"))
    menuToggle(); //pull the popout in when scrolling
  ...
  function toggleNav() {
    ...
    if (480 > document.body.clientWidth) {
      return;
    }
    menu.style.display =
      window.getComputedStyle(menu).display == "none" ? "flex" : "none";
...
function menuToggle() {
  ...
  window.getComputedStyle(modDis).display == "none"
    ? (modDis.style.display = "block")
    : (modDis.style.display = "none");
}
